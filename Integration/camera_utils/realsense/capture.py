...
#image saving stuff plz dont touch UwU
img_counter = 293
path = '/home/asabe/Desktop/asabeRobot/realsense/data/images'
# Streaming loop
try:
    while True:
	...
	#capture image if space bar is pressed
        if k%256 == 32:
                ...
                depthName="depth_{}.npy".format(img_counter)
                cv2.imwrite(os.path.join(path,imgName1), color_image)
                cv2.imwrite(os.path.join(path,imgName2), depth_colormap)
                np.save(os.path.join(path,depthName),depth_image)
