...
Cotton harvest will get the background, stem and pingpang ball out of img, just leave the connected cotton boll,
 and select the centerpoint of biggest connected part as the target
 '''
...
vs1 = np.hstack((color_image, img))  
vs2 = np.hstack((binary, output))  
result = np.vstack((vs1, vs2))
cv.imshow("result", result)
        
        #print('stats = ',stats)
[p,q]=np.int_(centroids[1])
print('num_labels = ',num_labels)
print('centroids = ', centroids)
print('top cotton point = ',np.int_(centroids[1]))
