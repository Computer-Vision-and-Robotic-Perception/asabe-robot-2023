...
def pole_assign(frame):
    ...
    method = 0 # method 1 = center of line for matching, method = 0 ends of lines for matching
    ...
    # Attempt at integration arc detection code for overlapping balls
    ...
    # arcmask = cv2.bitwise_and(lmao, lmao, mask = GreenMask)
    # img = cv2.cvtColor(arcmask, cv2.COLOR_BGR2GRAY)
    
    # edgez = cv2.Canny(img, 50, 170, apertureSize=3)
    # #
    # accuracy = 2
    # #
    # threshold = 200
    
    # _, threshz = cv2.threshold(edgez, 150, 255, cv2.THRESH_TOZERO)
    
    # dp = 2
    
    # minDist = 30
    
    # circles = cv2.HoughCircles(threshz, cv2.HOUGH_GRADIENT, dp, minDist, param1=70, param2=30, minRadius=0, maxRadius=40)
    
    # circles = np.uint16(np.around(circles))
    
    # for i in circles[0, :]:
    
    #     cv2.circle(lmao, (i[0], i[1]), i[2], (0, 255, 0), 2)
    
    #     cv2.circle(lmao, (i[0], i[1]), 2, (0, 0, 255), 2)
    
    # print(circles)
    
    # cv2.imshow("mask", lmao)
    # cv2.waitKey(0) 
    
    # cnts = cv2.findContours(circles.copy(), cv2.RETR_EXTERNAL,
    # 	cv2.CHAIN_APPROX_SIMPLE)
    # cnts = imutils.grab_contours(cnts)
