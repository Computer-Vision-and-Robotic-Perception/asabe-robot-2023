...
# usb_disk_path = '/home/zhengkun/asabe_ws/src/py_pubsub/py_pubsub/'
...
class RobotControl(Node):
    ...
    def rotate_angle(self, angle, angular_z = 0.5):
        twist = Twist()
...
def main(args=None):
    ...
    # start point: move with 45 degree, moving distance in x and y are 0.3m
    control_node.move_distance(distance = 0.61198, linear_x = 0.1, linear_y = 0)

    # clockwise 45 degree to the 2nd row
    control_node.rotate_angle(angle = 90, angular_z = -0.1)

    # move the robot along the 2nd row
    control_node.move_distance(distance = 1.8121, linear_x = 0.1, linear_y = 0 )

    # for i in range(9):
    #     control_node.move_distance(distance=0.127, linear_x=0.05, linear_y=0)  # Move a fraction of the total distance

    #     boll_detection = BollDetection()
    #     boll_count = boll_detection.get_center_points_count()
    #     change_csv_value(file_path, 1, i + 1, boll_count)
    #     time.sleep(1)  # Stop for 1 second

    # move the robot to the 4th row
    control_node.rotate_angle(angle = 90, angular_z = 0.1)
    control_node.move_distance(distance = 0.61198, linear_x = 0.1, linear_y = 0 )
    control_node.rotate_angle(angle = 90, angular_z = 0.1)

    # move the robot along the 4th row
    control_node.move_distance(distance = 1.8121, linear_x = 0.1, linear_y = 0 )
    # for i in range(9):
    #     control_node.move_distance(distance=0.127, linear_x=0.05, linear_y=0)  # Move a fraction of the total distance
    ...
    #     boll_detection = BollDetection()
    #     boll_count = boll_detection.get_center_points_count()
    #     change_csv_value(file_path, 2, i + 1, boll_count)
    #     time.sleep(1)  # Stop for 1 second

    # move the robot to the 6th row
    control_node.rotate_angle(angle = 90, angular_z = 0.1)
    control_node.move_distance(distance = 0.61198, linear_x = 0.1, linear_y = 0 )
    control_node.rotate_angle(angle = 90, angular_z = 0.1)

    # move the robot along the 6th row
    control_node.move_distance(distance = 1.8121, linear_x = 0.1, linear_y = 0)
    # for i in range(9):
    #     control_node.move_distance(distance=0.127, linear_x=0.05, linear_y=0)  # Move a fraction of the total distance
    ...
    #     boll_detection = BollDetection()
    #     boll_count = boll_detection.get_center_points_count()
    #     change_csv_value(file_path, 3, i + 1, boll_count)
    #     time.sleep(1)  # Stop for 1 second
